...
void RecupIndice( GtkWidget *pWidget, GtkComboBox *lTemps); //fonction de recupertation de donnée <- pour les minutes entre sauvegarde
...
void GTK (void)
{
	...
	//g_signal_connect(G_OBJECT(pButton[5]),"clicked",G_CALLBACK(RecupIndice), pLabel);
	//g_signal_connect(G_OBJECT(pButton[6]),"clicked",G_CALLBACK(RecupIndice), pLabel);
...
void RecupIndice(GtkWidget *pWidget, GtkComboBox *lTemps)
{
	gint value;
	value = gtk_combo_box_get_active(GTK_COMBO_BOX(lTemps)) +1 ;	
	printf("%d\n",value);		
