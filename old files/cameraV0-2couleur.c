...
int LowH =0,HighH =255,LowS =0,HighS=255,LowV=0,HighV=255;
...
static void onMouse( int event, int x, int y, int f, void* ){
 ...
 LowV=pix.val[2];
waitKey();
 printf("H= %d; S= %d; V= %d;",LowH,LowS,LowV);
}
...
{	
	...
	//on recupere la couleur que nous voulons detecter
	//setMouseCallback("flux_video", onMouse, 0 );
	...
	//printf("H%d S%d V%d H%d S%d V%d",LowH,LowS,LowV,HighH,HighS,HighV);
